   ...
   Modules:
       -Scanner:
           ...
           -Functions:
               ...
               -GetFunctionStart: Gets the start of the function with imagebase;
...
namespace Memcury
{
    ...
    public:
        ...
        static auto FindStringRef(T string, uintptr_t moduleBase = PE::GetModuleBase(), bool bThrow = true) -> Scanner
        {
            ...
            PE::Section dataSection{};
            bool hasDataSection = false;
            try {
                dataSection = PE::Section::GetSection(".data", moduleBase);
                hasDataSection = true;
            }
            catch (...) {}
            ...
            auto isValidStringSection = [&](PE::Address addr) -> bool {
                if (rdataSection.isInSection(addr, moduleBase)) {
                    return true;
                }
                if (hasDataSection && dataSection.isInSection(addr, moduleBase)) {
                    return true;
                }
                return false;
                };

            auto checkAndMatch = [&](PE::Address stringAdd, const std::uint8_t* aids) -> bool {
                bool matched = false;
                if constexpr (!bIsPtr)
                {
                    typedef typename T::value_type char_type;
                    matched = (T(stringAdd.GetAs<char_type*>()) == string);
                }
                else if constexpr (bIsWide) {
                    matched = (wcscmp(string, stringAdd.GetAs<const wchar_t*>()) == 0);

                }
                else {
                    matched = (strcmp(string, stringAdd.GetAs<const char*>()) == 0);
                }
                if (matched) {
                    add = PE::Address(reinterpret_cast<uintptr_t>(aids));
                }
                return matched;
                };

            for (DWORD i = 0x0; i < textSection.GetSectionSize() - 10; i++)
            {
                if (scanBytes[i] >= 0x40 && scanBytes[i] <= 0x4F)
                {
                    if (scanBytes[i + 1] == ASM::LEA && (scanBytes[i + 2] & 0b11000111) == 0b00000101)
                    {
                        int32_t disp = *reinterpret_cast<const int32_t*>(&scanBytes[i + 3]);
                        PE::Address stringAdd{ reinterpret_cast<uintptr_t>(&scanBytes[i + 7]) + disp };

                        if (isValidStringSection(stringAdd) && checkAndMatch(stringAdd, &scanBytes[i])) {
                            break;
                        }
                    }

                    if ((scanBytes[i] == 0x48 || scanBytes[i] == 0x49) && scanBytes[i + 1] >= 0xB8 && scanBytes[i + 1] <= 0xBF)
                    {
                        uint64_t absAddr = *reinterpret_cast<const uint64_t*>(&scanBytes[i + 2]);
                        PE::Address stringAdd{ static_cast<uintptr_t>(absAddr) };
